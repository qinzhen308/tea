...
public class DragSortListView extends ListView {
    ...
    /**
     * modify by paulz
     * 普通的事件传递流程在这里不适用。修改为，事件在child的消耗不影响此布局的事件，即在分发时就处理相应逻辑。
     * @param ev
     * @return
     */
    @Override
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (!isDragEnabled() || listViewIntercepted()) {
            return super.dispatchTouchEvent(ev);
        }
        saveTouchCoords(ev);
        mLastCallWasIntercept = true;
        ((DragSortController)mFloatViewManager).mDetector.onTouchEvent(ev);
        ((DragSortController)mFloatViewManager).flingTouch(ev);
        //child消耗时间之后，GestureDector的ontouch没法回调。所以要在分发前调用，否则本控件不能更新触摸时的位置。
        ((DragSortController)mFloatViewManager).onTouch(this,ev);
        //up的时候没有回调 onDragTouchEvent，导致在不move的情况下无法stopdrag
    ...
    protected boolean onDragTouchEvent(MotionEvent ev) {
        // we are in a drag
        int action = ev.getAction() & MotionEvent.ACTION_MASK;
        Log.d("drag","dslv---onDragTouchEvent--("+(int) ev.getX()+","+(int) ev.getY()+")");
