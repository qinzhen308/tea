...
public class DialogUtil {
    ...
    public static Dialog getLoadingDialog(Context context) {
        ...
        if (!((Activity) context).isFinishing()) {
            content = LayoutInflater.from(context).inflate(R.layout.image_buffer_layout, null);
            dialog = new Dialog(context, R.style.laoding_progress_dialog_style);
            dialog.setContentView(content, new ViewGroup.LayoutParams(-1, -1));
            dialog.setCancelable(true);
            Window window = dialog.getWindow();
            WindowManager.LayoutParams lp = window.getAttributes();
            lp.width = ScreenUtil.WIDTH;
            lp.height = ScreenUtil.HEIGHT;
        }
        return dialog;
    }
}
